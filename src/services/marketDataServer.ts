 ...
 * Handles Yahoo Finance fetching without CORS restrictions
 */
...
const CACHE_DURATION = 15 * 60 * 1000 // 15 minutes for real data
const SYNTHETIC_CACHE_DURATION = 60 * 60 * 1000 // 1 hour for synthetic data

/**
 * Fetch market data from Yahoo Finance (server-side only)
 */
...
/**
 * Get market data for a symbol with intelligent fallbacks (server-side)
 */
export async function getMarketDataServer(
  symbol: string, 
  days: number = 30,
  preferRealData: boolean = true
): Promise<MarketDataResult> {
  const cacheKey = `${symbol}_${days}_${preferRealData}`
  ...
  let result: MarketDataResult = {
    ...
    metadata: { originalSymbol: symbol }
  }
  
  // Try to get real data first if preferred
  if (preferRealData) {
    const proxy = SYMBOL_PROXIES[symbol]
    if (proxy) {
      try {
        console.log(`Server: Attempting to fetch real data for ${symbol} using proxy ${proxy.yahoo}`)
        const realData = await fetchYahooFinanceData(proxy.yahoo, days)
        ...
        if (realData.length > 0) {
          result = {
            ...
            metadata: {
              ...
              proxySymbol: proxy.yahoo,
              explanation: proxy.explanation
            }
          }
          ...
          console.log(`✅ Server: Successfully fetched ${realData.length} data points for ${symbol} via ${proxy.yahoo}`)
          return result
        }
      } catch (error) {
        console.warn(`Server: Failed to fetch real data for ${symbol}:`, error)
      }
    }
  }
  
  // Fallback to enhanced synthetic data
  console.log(`Server: Generating enhanced synthetic data for ${symbol}`)
  ...
  result = {
    ...
    metadata: {
      originalSymbol: symbol,
      explanation: `Enhanced synthetic data based on ${symbol} market characteristics`
    }
  }
  
  // Cache the synthetic result with longer duration
  dataCache.set(cacheKey, {
    data: result,
    expiry: Date.now() + SYNTHETIC_CACHE_DURATION
  })
  
  console.log(`✅ Server: Generated ${syntheticData.length} synthetic data points for ${symbol}`)
  return result
}
