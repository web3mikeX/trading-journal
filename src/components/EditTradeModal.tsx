...
interface EditTradeModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (tradeId: string, updates: any) => Promise<void>
  trade: Trade | null
}

export default function EditTradeModal({ isOpen, onClose, onSubmit, trade }: EditTradeModalProps) {
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    try {
      ...
      await onSubmit(trade.id, updates)
      onClose()
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : "Failed to update trade")
    } finally {
          ...
          <CardContent>
            <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
              ...
              <div className="border-t border-white/10 pt-4">
                <h3 className="text-lg font-semibold text-white mb-3">Exit Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
